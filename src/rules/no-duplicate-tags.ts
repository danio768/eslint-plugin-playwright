...
import {
  getAllSpecFiles as getSpecFiles,
  readFileContent,
} from '../utils/fileSystem.js'
import { parseFnCall } from '../utils/parseFnCall.js'
import {
  extractNumericTagsFromText,
...
export default createRule({
    ...
    return {
      ...
      'Program:exit'() {
        ...
        const otherFiles = allSpecFiles.filter(
          (file: string) => path.resolve(file) !== path.resolve(filename),
        )
        ...
        for (const { node, tag } of currentFileTags) {
          ...
          if (seenInFile.size < currentFileTags.length) {
            ...
            if (
              uniqueTagsInFile.size < currentFileTags.length &&
              seenInFile.has(tag)
            ) {
  ...
  meta: {
    docs: {
      description:
        'Prevent duplicate @number tags within and across .spec.ts files',
      recommended: true,
    },
    messages: {
