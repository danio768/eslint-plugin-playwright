...
// src/rules/no-duplicate-tags.ts
var path2 = __toESM(require("path"), 1);

// src/utils/fileSystem.ts
var fs = __toESM(require("fs"), 1);
var path = __toESM(require("path"), 1);
function getAllSpecFiles(projectRoot) {
  const files = [];
...
function getActualMaxDepth(startDir) {
  let maxDepthFound = 0;
  const exploreDir = (dir, currentDepth) => {
    try {
...
function scanTestDirectories(maxDepth, excludedDirs, files, projectRoot) {
...
function findTagPropertyNode(node) {
  const tagProperty = node.properties.find(
    (prop) => prop.type === "Property" && !("argument" in prop) && prop.key.type === "Identifier" && prop.key.name === "tag"
  );
...
function extractNumericTagsFromText(text) {
  ...
  for (const match of tagMatches) {
    if (match.includes("[")) {
      ...
      const arrayTags = arrayContent.split(",").map((t) => t.replace(/['"`]/g, "").trim()).filter(Boolean).filter((tag) => /^@\d+$/.test(tag));
      tags.push(...arrayTags);
    } else {
...
// src/rules/no-duplicate-tags.ts
var no_duplicate_tags_default = createRule({
  create(context) {
    const options = context.options[0] || {};
    ...
    return {
      ...
      "Program:exit"() {
        ...
        const otherFiles = allSpecFiles.filter(
          (file) => path2.resolve(file) !== path2.resolve(filename)
        );
        for (const { node, tag } of currentFileTags) {
...
// src/rules/require-test-tags.ts
var require_test_tags_default = createRule({
  create(context) {
    const options = context.options[0] || {};
    ...
    const isSharedPath = sharedPaths.some(
      (shared) => new RegExp(`(^|[\\\\/])${shared}([\\\\/]|$)`, "i").test(filename)
    );
    if (isSharedPath) {
      return {};
    }
    const isExcluded = (tag, excludes = []) => {
...
var index = {
  ...
  rules: {
    ...
    "no-duplicate-tags": no_duplicate_tags_default,
    ...
    "require-test-tags": require_test_tags_default,
